#ifndef TauAnalysis_SVfitStandalone_SVfitStandaloneLikelihood2b2tau_h
#define TauAnalysis_SVfitStandalone_SVfitStandaloneLikelihood2b2tau_h
...
namespace svFitStandalone2b2tau
{
  ...
  */
  enum kNLLParams2b2tau {    
    ...
    kBJet1Et,              /* < transverse energy of first b-jet                                                 */
    kBJet2Et,              /* < transverse energy of second b-jet                                                */
    kMaxNLLParams          /* < max number of parameters used for for the combined likelihood                    */
  };
  /**
     \class   MeasuredBJet SVfitStandaloneLikelihood2b2tau.h "TauAnalysis/SVfit2b2tau_standalone/interface/SVfitStandaloneLikelihood2b2tau.h"
     
     \brief   Helper class to simplify the configuration of the SVfitStandaloneLikelihood2b2tau class. 
     
     This is a helper class to facilitate the configuration of the SVfitStandaloneLikelihood2b2tau class. It keeps the spatial momentum
  ...
  */
  ...
  using svFitStandalone::roundToNdigits;
  ...
  class MeasuredBJet
  {
   public:
    ...
    /// constructor from the measured quantities 
    MeasuredBJet(double pt, double eta, double phi, double mass) 
      : pt_(roundToNdigits(pt)),
        eta_(roundToNdigits(eta)),
        phi_(roundToNdigits(phi)),
        mass_(roundToNdigits(mass))
    {
      //std::cout << "<MeasuredBJet>: Pt = " << pt_ << ", eta = " << eta_ << ", phi = " << phi_ << ", mass = " << mass_ << std::endl;
      initialize();
    }
    /// copy constructor
    MeasuredBJet(const MeasuredBJet& measuredBJet)
      : pt_(measuredBJet.pt()),
        eta_(measuredBJet.eta()),
        phi_(measuredBJet.phi()),
        mass_(measuredBJet.mass())
    {
      initialize();
    }
    /// default destructor
    ...
    /// return transverse energy of the measured b-jet in labframe
    double Et() const { return Et_; }
    /// return pt of the measured b-jet in labframe
    double pt() const { return pt_; }
    /// return px of the measured b-jet in labframe
    double px() const { return px_; }
    /// return py of the measured b-jet in labframe
    double py() const { return py_; }
    /// return pz of the measured b-jet in labframe
    double pz() const { return pz_; }
    /// return mass of the measured b-jet in labframe
    double mass() const { return mass_; }    
    /// return energy of the measured b-jet in labframe
    double energy() const { return energy_; }
    /// return momentum of the measured b-jet in labframe
    double momentum() const { return momentum_; }
    /// return pseudo-rapidity of the measured b-jet in labframe
    double eta() const { return eta_; }
    /// return azimuthal angle of the measured b-jet in labframe
    ...
    /// return the direction in the labframe
    Vector direction() const { return direction_; }
    
   private:
    /// energy and momentum in labframe 
    double Et_;
    double pt_;
    double eta_;
    double phi_;
  ...
  /**
   \class   SVfitStandaloneLikelihood2b2tau SVfitStandaloneLikelihoodd2b2tau .h "TauAnalysis/SVfitStandaloned2b2tau /interface/SVfitStandaloneLikelihoodd2b2tau .h"
       
     \brief   Negative log likelihood for a decay to two Higgs bosons into two tau leptons + two b-jets.
              The tau leptons may decay hadronically or leptonically.
     
     Negative log likelihood for a decay to two Higgs bosons into two tau leptons + two b-jets.
     The tau leptons may decay hadronically or leptonically
     ...
     \var measuredBJets      : the vector of the two reconstructed b-jets
     ...
     The SVfitStandaloneLikelihood2b2tau class is for internal use only. The general use calse is to access it from the class 
     SVfitStandaloneAlgorithm2b2tau as defined in interface/SVfitStandaloneAlgorithm2b2tau.h in the same package. The SVfitLikelihood2b2tau class 
  ...
  */
  ...
  class SVfitStandaloneLikelihood2b2tau
  {
   public:
    /// error codes that can be read out by SVfitAlgorithm2b2tau
    enum ErrorCodes {
      None            = 0x00000000,
      MatrixInversion = 0x00000001,
      LeptonNumber    = 0x00000010,
      BJetNumber      = 0x00000100,
    };
    /// constructor with a minimla set of configurables 
    SVfitStandaloneLikelihood2b2tau(const std::vector<svFitStandalone::MeasuredTauLepton>&, const svFitStandalone::Vector&, const TMatrixD&, const std::vector<svFitStandalone2b2tau::MeasuredBJet>&, bool);
    /// default destructor
    ~SVfitStandaloneLikelihood2b2tau() {}
    /// static pointer to this (needed for the minuit function calls)
    static const SVfitStandaloneLikelihood2b2tau* gSVfitStandaloneLikelihood2b2tau;
    ...
    /// accessor functions to properties of measuredTauLeptons and measuredBJets,
    /// called by SVfitStandaloneAlgorithm2b2tau
    double diTauMass_unfitted() const { return diTauMass_unfitted_; }
    double diBJetMass_unfitted() const { return diBJetMass_unfitted_; }
    double bJet1Et_unfitted() const { return bJet1Et_unfitted_; }
    double bJet2Et_unfitted() const { return bJet2Et_unfitted_; }
    ...
    void results2tau(std::vector<svFitStandalone::LorentzVector>& fittedTauLeptons, const double* x) const;
    /// return vector of measured b-jets
    const std::vector<svFitStandalone2b2tau::MeasuredBJet>& measuredBJets() const { return measuredBJets_; }
    /// return vector of fitted b-jets, which will be the actual fit result. This function is a subset of transform.
    /// It needs to be factored out though as transform has to be const to be usable by minuit and therefore is not allowed 
    /// change the class members.  
    void results2b(std::vector<svFitStandalone::LorentzVector>& fittedBJets, const double* x) const;
   ...
   protected:
    ...
    /// measured visible decay products of the two tau leptons
    ...
    double covDet_;
    /// measured b-jets
    std::vector<svFitStandalone2b2tau::MeasuredBJet> measuredBJets_;
    ...
    /// resolution on transverse energy of b-jets
    BJetEtResolution bJetEtResolution_;  
  };
}
